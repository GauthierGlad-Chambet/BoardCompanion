<main>
    ...
    <form class="container form-finalReport"  action="" method="POST">
            ...
            <?php foreach ($arrAppreciations as $appreciation) : ?>
                <div>
                    <label class="customLabel radioLabel" for="appreciation_<?= $appreciation->getId() ?>">
                        <input type="radio" id="appreciation_<?= $appreciation->getId() ?>" name="appreciation" value="<?= $appreciation->getId() ?>" required />
        ...
        <div>
            ...
            <input type="number" step="0.1" min="0" id="duree_totale_projet" name="duree_totale_projet" required>
        </div>
        <div>
            <label for="total_plans"><h3>Total de plans réalisés :</h3></label>
            <input type="number" min="0" id="total_plans" name="total_plans">
        </div>
        <div>
            <?php if ($project->getIs_cleaning() == 1) { ?>
            <label for="duree_cleaning"><h3>Durée du cleaning (en jours) :</h3></label>
            <input type="number" id="duree_cleaning" step="0.1" min="0" id="duree_cleaning" name="duree_cleaning">
            <?php } ?>
        </div>
        ...
        <div class="container-sequences">
            ...
            <div class="all-sequences-finalReport">
                ...
                <b>Séquence <?= $sequence->getNumber() ?> :</b></br><?= $sequence->getTitle() ?> (<?= $sequence->getTypeLabel() ?>)
                    ...
                    <input type="number" min="0" id="duree_sequence[<?= $sequence->getId();?>]" name="duree_sequence[<?= $sequence->getId();?>]">
                    <br>
                <?php }} ?>
            </div>
        </div>
