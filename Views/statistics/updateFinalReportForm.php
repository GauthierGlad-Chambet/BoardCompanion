<main>
    ...
    <form class="form-select" action="index.php" method="get">
        ...
        <input type="hidden" name="action" value="updateFinalReport">
    ...
    <form class="container form-finalReport" action="" method="POST">
            ...
            <?php foreach ($arrAppreciations as $appreciationObj) : ?>
                <div>
                    <label class="customLabel radioLabel" for="appreciation_<?= $appreciationObj->getId() ?>">
                        <input type="radio" id="appreciation_<?= $appreciationObj->getId() ?>" name="appreciation" value="<?= $appreciationObj->getId() ?>" <?=
                        ($appreciation == $appreciationObj->getId()) ? 'checked' : ($finalReport->getFk_appreciation() == $appreciationObj->getId() ? 'checked' : '') ?> required />
        ...
        <div>
            ...
            <input <?php if (!empty($_SESSION['error']['duree_totale_projet'])) { ?> class="inputError" <?php } ?> type="number" id="duree_totale_projet" name="duree_totale_projet" value="<?= ($duree_totale_projet !== '')? $duree_totale_projet : $finalReport->getTotal_duration() ?>" step="0.1" min="0" required>
        ...
        <div>
            ...
            <input <?php if (!empty($_SESSION['error']['total_plans'])) { ?> class="inputError" <?php } ?> type="number" id="total_plans" name="total_plans" value="<?= ($total_plans !== '')? $total_plans : $finalReport->getNb_shots() ?>" min="0">
        ...
        <div>
            ...
            <input <?php if (!empty($_SESSION['error']['duree_cleaning'])) { ?> class="inputError" <?php } ?> type="number" id="duree_cleaning" name="duree_cleaning" value="<?= ($duree_cleaning !== '')? $duree_cleaning : $finalReport->getCleaning_duration() ?>" step="0.1" min="0">
        ...
        <div>
            ...
            <textarea name="commentaire" id="commentaire" placeholder="Commentaire…"><?= ($commentaire !== '')? $commentaire : $finalReport->getCommentary() ?></textarea>
        </div>
